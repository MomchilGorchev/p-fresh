...
function Util(){
    ...
    _this.isSafari = /^((?!chrome).)*safari/i.test(navigator.userAgent);

    // And IE
    _this.isIe = /MSIE 9/i.test(navigator.userAgent) || /MSIE 10/i.test(navigator.userAgent) || /rv:11.0/i.test(navigator.userAgent);

    //Mobile device detection - taken from http://detectmobilebrowsers.com/
    _this.mobileDevice = function(){
        var check = false;
    ...
    _this.disableSVG = function(){
        var svgHeaders = document.querySelectorAll('.svg-header'),
            svgCount = svgHeaders.length;
        for(var h = 0; h < svgCount; h++){
            svgHeaders[h].style.opacity = 0;
        }
    };
